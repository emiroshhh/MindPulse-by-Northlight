...
import { acquisitionPageCopyFor, type AcquisitionPageId } from '@/lib/mindpulse/acquisition-page-i18n';
import type { PublicPageId } from '@/lib/mindpulse/public-page-i18n';
import { absoluteSiteUrl, localizedMarketingPath, type MarketingLocale, type IndexableMarketingPath } from '@/lib/seo';
...
export function StaticAcquisitionPage({
  ...
  const items = [
    { name: 'MindPulse', url: absoluteSiteUrl(localizedMarketingPath(locale, '/')) },
    { name: copy.title, url: absoluteSiteUrl(localizedMarketingPath(locale, marketingPath)) },
  ];
  return (
    <>
